...
| `anthropic-auth` | provider auth | **separate nested repo** (`@cortexkit/anthropic-auth`): own bun workspace, own build/test/biome, packages/{core,pi,opencode}. Not covered by root tsconfig or pnpm check. Only entry wired into `settings.json` (`./extensions/anthropic-auth/packages/pi`) because it needs the built package |
...
- pi API docs: `/Users/hau/dev/pi/packages/coding-agent/docs/extensions.md` + `docs/tui.md`, examples in
  `.../examples/extensions/`.
