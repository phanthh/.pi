...
function analyzeSegment(seg: Token[]): Risk | null {
	...
	if (cmd === "kubectl") {
		const sub = rest[0];
		const destructiveSubs: Record<string, string> = {
			delete: "resource deletion",
			drain: "node drain (evicts pods)",
			cordon: "node cordon (marks unschedulable)",
			uncordon: "node scheduling change",
			taint: "node taint (can evict pods)",
			replace: "resource replacement (can recreate objects)",
			scale: "replica scaling (can kill pods)",
			patch: "in-place resource mutation",
			evict: "pod eviction",
		};
		if (sub && sub in destructiveSubs) {
			severity = "high";
			reasons.push(`kubectl ${sub} (${destructiveSubs[sub]})`);
		}
		if (sub === "apply" && (rest.includes("--force") || rest.includes("--prune"))) {
			severity = "high";
			reasons.push("kubectl apply --force/--prune (can delete/recreate resources)");
		}
		if (sub === "rollout" && rest[1] === "undo") {
			severity = "high";
			reasons.push("kubectl rollout undo (deployment rollback)");
		}
		if ((sub === "label" || sub === "annotate") && rest.includes("--overwrite")) {
			severity = severity === "high" ? "high" : "medium";
			reasons.push(`kubectl ${sub} --overwrite (metadata mutation)`);
		}
	}
	...
	if (cmd === "gcloud") {
		reasons.push("gcloud (Google Cloud CLI command)");
		const destructiveVerbs = ["delete", "destroy", "remove", "reset", "abandon-instances", "rollback", "purge", "revoke", "stop", "suspend", "detach-disk", "unset"];
		const hit = rest.find((a) => destructiveVerbs.includes(a) || a.startsWith("remove-") || a.startsWith("delete-"));
		if (hit) {
			severity = "high";
			reasons.push(`gcloud ${hit} (destructive cloud operation)`);
		}
		if (rest.includes("--quiet") || rest.includes("-q")) {
			reasons.push("gcloud --quiet (skips confirmation prompts)");
		}
	}
...
// Subagent detection: tmux-subagents sets PI_SUBAGENT_DEPTH on launch and PI_SUBAGENT_ID on
// both launch and resume. Any of them => spawned child (worker/scout/delegate/...).
// Guard is ON by default there (hard-block, no prompting); master/orchestrator starts OFF.
const _subagentDepth = Number(process.env.PI_SUBAGENT_DEPTH ?? "0");
const _isSubagent =
	(Number.isFinite(_subagentDepth) && _subagentDepth >= 1) || !!process.env.PI_SUBAGENT_ID;
...
const HEADLESS_BLOCKED: Array<{ pattern: RegExp; reason: string }> = [
	...
	{ pattern: /\bkubectl\s+(drain|taint|replace|evict)\b/, reason: "destructive Kubernetes node/resource operation" },
	{ pattern: /\bkubectl\s+apply\b[^#\n]*--(force|prune)\b/, reason: "kubectl apply --force/--prune (can delete/recreate resources)" },
	{ pattern: /\bgcloud\b[^#\n]*\b(delete|destroy|remove(?:-\w+)?|reset|rollback|purge|revoke)\b/, reason: "destructive gcloud operation" },
...
// Hard-block floor for the main (master/orchestrator) session, where bash-guard is
// disabled by default. Master is attended by the user, so routine work passes through:
// git ops, `rm -r`, `sudo`, pipe-to-shell are allowed. Only irreversible machine-level
// destruction (format/partition/power/infra teardown) stays blocked.
...
export default function (pi: ExtensionAPI) {
	...
	// Main session mode: interactive prompting, DISABLED by default.
	...
	pi.registerFlag("bash-guard-enabled", {
		description: "Start the main session with bash-guard enabled (interactive prompting). Off by default.",
		type: "boolean",
		default: false,
	});

	// Session-local toggle. Master/orchestrator starts disabled (autonomous);
	// hard-block floor still applies. Not persisted across reloads or restarts.
	let disabled = true;

	pi.on("session_start", async (event, ctx) => {
		if (event.reason !== "startup") return;
		if (pi.getFlag("--bash-guard-enabled") === true) {
			disabled = false;
