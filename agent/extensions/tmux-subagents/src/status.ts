/**
 * Status classification for running subagents.
 *
 * Liveness comes from the child-written activity snapshot (activity.ts), never
 * from session-file growth. A run is `stalled` when valid snapshots never
 * appear, stop being readable, or stop matching the current child.
 */
export const SNAPSHOT_STALLED_AFTER_MS = 60_000;
export const STATUS_LINE_LIMIT = 4;
...
export type StatusObservation =
  ...
  | { snapshot: "missing" | "invalid" | "wrong-id"; snapshotError?: string };
...
function snapshotProblemLabel(snapshotState: StatusSnapshotState): string | null {
  return snapshotState === "wrong-id" ? "wrong activity id" : null;
}
...
export function observeStatus(
  ...
  const { lastActivityAtMs, lastActivitySequence } = state;
...
/** After an interrupt, pin the child to `waiting` until a newer snapshot lands. */
...
export function classifyStatus(state: SubagentStatusState, now: number): StatusSnapshot {
  ...
  if (state.snapshotState === "present") {
    if (state.phase === "active" || state.activeNow) {
      ...
      const referenceMs = state.firstObservationAtMs ?? state.startTimeMs;
      kind = Math.max(0, now - referenceMs) >= SNAPSHOT_STALLED_AFTER_MS ? "stalled" : "starting";
    }
  } else {
...
export function advanceStatusState(
  ...
  return { snapshot, transition, nextState: { ...state, currentKind: snapshot.kind } };
}

function formatActiveDetail(snapshot: StatusSnapshot): string {
  const label = activityLabel(snapshot);
  if (!label) return "active";
  return `active (${label}${snapshot.activeDurationText ? ` ${snapshot.activeDurationText}` : ""})`;
}

function formatWaitingDetail(snapshot: StatusSnapshot): string {
  return `waiting${snapshot.waitingDurationText ? ` ${snapshot.waitingDurationText}` : ""}`;
}
...
export function capStatusLines(lines: string[], lineLimit: number): CappedStatusLines {
  ...
  return { visibleLines, overflow: Math.max(0, lines.length - visibleLines.length) };
}
