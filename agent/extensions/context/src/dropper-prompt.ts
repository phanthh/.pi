// Ported from k0valik/pi-blackhole (MIT), src/om/agents/dropper/prompts.ts.
...
Your job is to identify only the safest active observations to remove from compacted memory by returning their ids as structured output. Default action is KEEP. When uncertain, keep the observation.
...
- You can only return ids from the current observations list.

Do not force drops you do not believe in. If no observations are safe to drop, emit an empty ids array. Hitting the budget or maximum count is less important than preserving load-bearing memory.`;
