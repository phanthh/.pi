...
export const PROBE_IDENTITIES_CUSTOM_TYPE = "context:probe-identities";
...
export class InitialCaptureState {
	...
	/** Add a provider-only message observed after this extension's context hook ran. */
	public observeLateInjectedMessage(message: unknown): void {
		this.initialSnapshot = mergeLateInjectedMessage(this.initialSnapshot, message);
	}
...
/**
 * Add one provider-only custom message reported after capture. Before capture,
 * the normal chained context event will contain it; repeated reports dedupe.
 */
export function mergeLateInjectedMessage(
	snapshot: InitialSnapshot | undefined,
	message: unknown,
): InitialSnapshot | undefined {
	if (snapshot === undefined || !isCustomContextMessage(message)) return snapshot;
	const [item] = measureInjectedMessages([message], []);
	if (item === undefined) return snapshot;
	const existing = snapshot.groups.flatMap((group) => group.items);
	if (existing.some((candidate) =>
		candidate.kind === "message"
		&& candidate.source.id === item.source.id
		&& candidate.text === item.text
	)) return snapshot;
	return buildSnapshot([...existing, item], snapshot.origin, snapshot.capturedAt);
}

/** Accept only the custom-message shape required by ContextEvent. */
function isCustomContextMessage(message: unknown): message is Extract<ContextEvent["messages"][number], { role: "custom" }> {
