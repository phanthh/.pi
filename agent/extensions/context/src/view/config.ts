 ...
 * `<agent dir>/extensions/context-view.json` carries overrides only. The
...
export const CONFIG_FILE_NAME = "context-view.json";
...
/** Absolute path of the global override file, falling back to a prior namespaced filename. */
export function getConfigFilePath(agentDir: string = getAgentDir()): string {
	const extensionsDir = join(agentDir, "extensions");
	const currentPath = join(extensionsDir, CONFIG_FILE_NAME);
	if (existsSync(currentPath)) return currentPath;
	try {
		const priorName = readdirSync(extensionsDir, { withFileTypes: true })
			.filter((entry) => entry.isFile() && entry.name.endsWith(`-${CONFIG_FILE_NAME}`))
			.map((entry) => entry.name)
			.sort()[0];
		return priorName === undefined ? currentPath : join(extensionsDir, priorName);
	} catch {
		return currentPath;
	}
}
