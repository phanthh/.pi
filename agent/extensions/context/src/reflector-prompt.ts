// Ported from k0valik/pi-blackhole (MIT), src/om/agents/reflector/prompts.ts.
...
Your task is different from the observer's: you are not recording events, you are distilling stable, long-lived facts and patterns from active observations into new reflections as structured output. Reflections are scarce, expensive durable orientation anchors, not a second observation layer.
...
- It is fine to emit zero reflections when nothing new is stable enough; in that case emit an empty reflections array.
