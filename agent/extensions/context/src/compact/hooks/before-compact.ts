...
import { loadSettings, type CompactorSettings } from "../core/settings.ts";
...
let lastCompactWasMarker = false;
...
// (https://github.com/monotykamary/pi-vcc, MIT) — an upstream derivative.
...
export const formatCompactionStats = (stats: CompactionStats): string => {
  if (stats.budgetCut) {
    ...
    return `compact: kept ~${formatTokens(stats.keptTokensEst)} tok tail (mid-turn cut, ${reason}), summarized ${stats.summarized}.`;
  }
  const notes: string[] = [`summarized ${stats.summarized}`];
  if (stats.smartKeepAdjusted) {
    notes.push("smart-keep");
  }
  return `compact: kept ${stats.keptUserTurns}/${stats.totalUserTurns} turns, ~${formatTokens(stats.keptTokensEst)} tok (${notes.join(", ")}).`;
};
...
const parseCompactionInstructions = (customInstructions?: string): {
  isMarkerCompact: boolean;
  keepUserTurns: number;
  keepUserTurnsExplicit: boolean;
  followUpPrompt: string | null;
} => {
  const trimmed = customInstructions?.trim();
  if (trimmed === COMPACT_MARKER) {
    return { isMarkerCompact: true, keepUserTurns: 1, keepUserTurnsExplicit: false, followUpPrompt: null };
  }

  const keepPrefix = `${COMPACT_MARKER} `;
  if (trimmed?.startsWith(keepPrefix)) {
    const parsed = parseKeepAndPrompt(trimmed.slice(keepPrefix.length));
    return {
      isMarkerCompact: true,
...
const dbg = (settings: CompactorSettings, data: Record<string, unknown>) => {
  if (!settings.debug) return;
  try { writeFileSync("/tmp/compact-debug.json", JSON.stringify(data, null, 2)); } catch {}
};
...
const REASON_MESSAGES: Record<OwnCutCancelReason, string> = {
  no_live_messages: "compact: Nothing to compact (no live messages)",
...
export const registerBeforeCompactHook = (pi: ExtensionAPI, options: CompactRegistrationOptions = {}) => {
  ...
  pi.on("session_before_compact", (event, ctx) => {
    ...
    // Always handle explicit marker.
    // Otherwise, only handle when user opted in via settings.
    const { isMarkerCompact, keepUserTurns, keepUserTurnsExplicit, followUpPrompt } = parseCompactionInstructions(customInstructions);
    ...
    if (!ownCut.ok) {
      ...
      pendingFollowUpPrompt = null;
      const fallbackToCore = !isMarkerCompact && (reason === "overflow" || willRetry);
      dbg(settings, {
    ...
    lastCompactWasMarker = isMarkerCompact;

    let finalSummary = summary;
    let finalDetails = details;
    if (options.enrichCompaction) {
      try {
        const enriched = options.enrichCompaction({ summary, details }, ctx);
        if (enriched?.summary) finalSummary = enriched.summary;
        if (enriched?.details) finalDetails = enriched.details;
      } catch {}
    }
  ...
  // Marker path uses its own onComplete callback in the command handler.
  pi.on("session_compact", async (event, ctx) => {
    const { reason, willRetry } = readCompactionEventContext(event);
    if (!event.fromExtension) return;
    const followUpPrompt = pendingFollowUpPrompt;
    pendingFollowUpPrompt = null;
    if (lastCompactWasMarker) return; // marker path handles its own toast via onComplete
    if (willRetry) return;
