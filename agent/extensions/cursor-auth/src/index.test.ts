...
describe("buildCursorRequest — turn reconstruction", () => {
  ...
  test("no checkpoint, with assistant-text turns — reconstructs protobuf turns without inline fallback", () => {
    ...
    const userAction = req.action.action.value as any;
    expect(userAction.userMessage.text).toContain("User: first question");
    expect(userAction.userMessage.text).toContain("Assistant: second answer");
  ...
  test("no checkpoint, reconstructs tool-call steps and final assistant text", () => {
    ...
    const userAction = req.action.action.value as any;
    expect(userAction.userMessage.text).toContain(
      'Assistant tool call: read({"path":"src/index.ts"})',
    );
    expect(userAction.userMessage.text).toContain("Tool result: file contents");
    expect(userAction.userMessage.text).toContain(
      "[Current user message]\n\nfix it",
    );
  });
  ...
  test("empty checkpoint falls back to parsed message history", () => {
    ...
    const turns = [turn("first question", [assistantStep("first answer")])];
    const payload = buildCursorRequest(
      "gpt-5",
      "system",
      "next",
  ...
  test("parsed message history replaces a checkpoint with history", () => {
    const priorPayload = buildCursorRequest(
      "gpt-5",
      "system",
      "hello",
      [turn("checkpoint question", [assistantStep("checkpoint answer")])],
      "conv-1",
      null,
    );
    const priorReq = decodeRunRequest(priorPayload);
    ...
    const payload = buildCursorRequest(
      ...
      "system",
      "next",
      [turn("Pi question", [assistantStep("Pi answer")])],
      "conv-1",
      checkpoint,
      priorPayload.blobStore,
    );
    const req = decodeRunRequest(payload);
    const decoded = decodeTurns(req.conversationState, payload.blobStore);

    expect(decoded).toHaveLength(1);
    expect(decoded[0]!.userMsg.text).toBe("Pi question");
    expect((decoded[0]!.steps[0]!.message.value as any).text).toBe("Pi answer");
  });

  test("system prompt is preserved without the Cursor-native tool policy", () => {
    const payload = buildCursorRequest(
      "gpt-5",
      "You are helpful",
      "hi",
    ...
    expect(blobData).toEqual({ role: "system", content: "You are helpful" });
  });

  test("checkpoint root prompt is refreshed without modifying it", () => {
    const priorPayload = buildCursorRequest(
      "gpt-5",
      "Old system prompt",
      "first",
    ...
    const priorReq = decodeRunRequest(priorPayload);
    priorReq.conversationState.rootPromptMessagesJson = [new Uint8Array([1])];
    const checkpoint = toBinary(
      ConversationStateStructureSchema,
      priorReq.conversationState,
    );
    ...
    expect(blobData.content).toBe("New system prompt");
  });
...
describe("fork discards checkpoint, reconstruction takes over", () => {
  test("fork scenario — checkpoint discarded, turns reconstructed from messages", () => {
    ...
    const userAction = req.action.action.value as any;
    expect(userAction.userMessage.text).toContain("User: first");
    expect(userAction.userMessage.text).toContain("Assistant: response1");
    expect(userAction.userMessage.text).toContain(
      "[Current user message]\n\nforked question",
    );
  });
...
function makeCheckpointMessage(turns: Uint8Array[] = []) {
  return create(AgentServerMessageSchema, {
    message: {
      case: "conversationCheckpointUpdate",
      value: create(ConversationStateStructureSchema, { turns }),
    },
  });
}
...
describe("proxy integration — session handling", () => {
  test("empty checkpoint reconstructs prior turns on the next session request", async () => {
    ...
    setBridgeFactoryForTests(
      (options) =>
        new FakeBridge(options, (clientMessage, fake) => {
          ...
          fake.emitServerMessage(makeTextDeltaMessage("first answer"));
    ...
    expect(runRequests[1].conversationState.turns).toHaveLength(1);
  });
  ...
  test("interrupt after a checkpoint reuses the stored checkpoint on the next request", async () => {
    ...
    interruptedBridge.emitServerMessage(
      makeCheckpointMessage([new Uint8Array([1])]),
    );
    await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    expect(runRequests).toHaveLength(1);
    const expectedState = fromBinary(
      ConversationStateStructureSchema,
      storedCheckpoint!,
    );
    expectedState.rootPromptMessagesJson =
      runRequests[0].conversationState.rootPromptMessagesJson;
    expect(
      toBinary(ConversationStateStructureSchema, runRequests[0].conversationState),
    ).toEqual(toBinary(ConversationStateStructureSchema, expectedState));
  ...
  test("interrupt after checkpoint reuses it even when pi includes partial assistant text in resumed history", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(runRequests).toHaveLength(1);
    // Checkpoint state is reused despite partial text; only root policy is refreshed.
    const expectedState = fromBinary(
      ConversationStateStructureSchema,
      storedCheckpoint!,
    );
  ...
  test("same-depth branch uses Pi's current message history", async () => {
    // Pi's request history is authoritative even if a stale Cursor checkpoint
    // exists at the same depth.
    const runRequests: any[] = [];
