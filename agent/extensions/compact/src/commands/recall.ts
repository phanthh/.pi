    ...
    handler: async (args: string, ctx) => {
      ...
      if (hits.length > 0 && page > totalPages) {
        ...
        const guidance = truncated
          ? `Use /recall ${query}${scopeArg} page:N with N between 1 and ${totalPages}.`
          : `Use /recall ${query}${scopeArg} page:N with N between 1 and ${totalPages}, or refine your query.`;
        const text =
          `Page ${page} is outside the available range 1-${totalPages} ` +
          `(${hits.length} matches${scopeSuffix}${truncationNote}). ${guidance}`;
        pi.sendMessage({ customType: "recall", content: text, display: true }, { triggerTurn: true });
        return;
      }
