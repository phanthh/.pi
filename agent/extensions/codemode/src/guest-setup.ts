 ...
 * globals a program sees: pi.*, extensions.*, tools.*, π, print.
...
// is evaluated inside QuickJS. Ported (trimmed) from pi-fabric core/pi-arguments.ts.
...
globalThis.pi = new Proxy({}, {
  get(_target, property) {
    ...
    return (...rest) => {
      const raw = rest.length <= 1 ? (rest.length === 1 ? rest[0] : {}) : __toArgs(name, rest);
      const promise = __call("pi." + name, __normalizePiArgs(name, raw === undefined ? {} : raw));
...
globalThis.extensions = new Proxy({}, {
  get(_target, property) {
    if (property === "then" || typeof property === "symbol") return undefined;
    const name = String(property);
    return (args = {}) => __call("ext." + name, args);
  },
});
...
// tools is discovery + generic calls only; tools.read(...) is a common miss.
globalThis.tools = new Proxy(__toolsBase, {
  get(target, property) {
    if (property === "then" || typeof property === "symbol") return undefined;
    const name = String(property);
    if (__coreTools.indexOf(name) >= 0) {
      return () => {
        throw new Error(
          "tools." + name + " does not exist. tools is discovery only (list/describe/call). " +
          "For the Pi core tool call pi." + name + "(args)."
        );
      };
    }
    return target[name];
