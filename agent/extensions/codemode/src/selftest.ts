...
const hostCall: HostCall = async (ref, args) => {
	...
	if (ref === "ext.echo_tool") return { echoed: args.message };
...
const tests: Array<[string, () => Promise<void> | void]> = [
	...
	[
		...
		async () => {
			...
			await assert.rejects(
				() => dispatcher.hostCall("ext.echo_tool", { times: 2 }, signal),
				/Invalid arguments/,
			);
			assert.equal(executed, false, "invalid args must not reach the tool");
			const ok = await dispatcher.hostCall("ext.echo_tool", { message: "hi" }, signal);
	...
	[
		"tools.<coreTool> points at pi.<coreTool>",
		async () => {
			const outcome = await run(`return await (tools as any).read({ path: "a" });`);
			assert.match(String(outcome.result?.error), /pi\.read/);
		},
	],
	[
		...
		async () => {
			...
			const result = await execute(`return await (tools as any).call({ ref: "sleep" });`, hostCall, {
				timeoutMs: 300,
				memoryLimitBytes: 64 * 1024 * 1024,
			});
