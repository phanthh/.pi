...
The program is an async function body: top-level await and return are supported. Globals:
  pi.<tool>(args)          — every core and extension tool
  pi.list/describe/call    — discovery, plus calling a ref computed at runtime
...
export default function codeMode(pi: ExtensionAPI): void {
	...
	// works, so degrade to core tools only instead of taking Pi down.
