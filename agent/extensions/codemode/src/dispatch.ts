...
export const createDispatcher = (options: DispatchOptions) => {
	...
	const resolve = (ref: string): { name: string; definition: AnyToolDefinition } | undefined => {
		if (ref.startsWith("pi.")) {
			...
			const definition = core[name as CoreToolName];
			return definition ? { name, definition } : undefined;
		}
		if (ref.startsWith("ext.") || ref.startsWith("extensions.")) {
			const name = ref.slice(ref.indexOf(".") + 1);
			const tool = options.captured.get(name);
			return tool ? { name, definition: tool.definition } : undefined;
		}
		return undefined;
	};
	...
	const refs = (): ToolRefInfo[] => [
		...
		...options.captured.list().map((tool) => ({
			ref: `extensions.${tool.definition.name}`,
	...
	const invoke = async (
		...
		// tools.call({ ref: "pi.read", ... }) skips the guest's pi proxy, so the same
		// repair has to run here or a runtime-computed ref fails the schema gate
		// on aliases that the direct proxy accepts.
		const repaired = ref.startsWith("pi.")
			? (normalizePiArgs(name, args) as Record<string, unknown>)
			: args;
		...
		if (!valid) {
			const problems = [...Value.Errors(schema, prepared)]
				...
				.join("; ");
			throw new Error(
				`Invalid arguments for ${ref}: ${problems}. Call tools.describe({ ref: "${ref}" }) and read inputSchema, then retry.`,
			);
		}
		nestedCalls += 1;
